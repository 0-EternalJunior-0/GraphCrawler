...
def graph_to_dict(graph: Graph, include_metadata: bool = True) -> Dict[str, Any]:
    ...
    """
    nodes_data = []
    # Використовуємо iter_nodes() для streaming доступу (memory-efficient)
...
def filter_nodes_for_visualization(
    ...
    if len(nodes_list) > max_nodes:
        ...
        if priority_attributes:
            logger.info(
                " Фільтрація: %s структурних, %s пріоритетних, %s інших -> %s всього",
                len(structural),
                len(priority_nodes),
                len(other_url_nodes),
                len(result),
...
def _visualize_via_subprocess(
    ...
    for cmd in python_paths:
        try:
            ...
            if result.returncode == 0 and "3.14" not in result.stdout:
                python_cmd = cmd
                logger.info(" Знайдено сумісний Python: %s", result.stdout.strip())
                break
        except (subprocess.SubprocessError, FileNotFoundError, OSError):
            continue
        except Exception:
            continue  # Fallback для невідомих помилок
    ...
    check_pyvis = subprocess.run(
        python_cmd + ["-c", "import pyvis; print('OK')"], capture_output=True, text=True, timeout=10
    )
    ...
    if "OK" not in check_pyvis.stdout:
        ...
        if install_result.returncode != 0:
            logger.warning(" Не вдалося встановити pyvis: %s", install_result.stderr)
            return False
...
"""
    ...
    try:
        ...
        result = subprocess.run(
            python_cmd + ["-c", viz_script], capture_output=True, text=True, timeout=60
        )
        ...
        if "SUCCESS" in result.stdout:
            logger.info(" Візуалізацію збережено: %s", output_file)
            print(f"✅ Візуалізацію збережено: {output_file}")
            return True

        if result.stderr:
            logger.warning("Subprocess error: %s", result.stderr)
        return False
...
def visualize_2d_web(
    ...
    try:
        ...
        if "__annotate__" in str(exc):
            ...
            logger.warning("Python %s несумісний з networkx, спробуємо через subprocess...", py_ver)
