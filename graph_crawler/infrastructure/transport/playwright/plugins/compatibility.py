...
class CompatibilityReport:
    ...
    def has_conflicts(self) -> bool:
        ...
        return any(issue.level == CompatibilityLevel.INCOMPATIBLE for issue in self.issues)
...
class PluginCompatibilityChecker:
    ...
    def __init__(
        self, compatibility_matrix: Optional[Dict] = None, supersedes: Optional[Dict] = None, strict_mode: bool = False
    ):
        """
        Args:
    ...
    def _check_pair(self, name1: str, name2: str, configs: Dict) -> Optional[CompatibilityIssue]:
    ...
    def _check_config_conflicts(
        self, plugin_names: Set[str], configs: Dict, report: CompatibilityReport
    ):
...
def check_plugin_compatibility(
    plugins: List, raise_on_conflict: bool = True
) -> CompatibilityReport:
    """
    ...
    if report.has_warnings:
        logger.warning("Plugin compatibility warnings:\n%s", report.format_report())
...
def get_stealth_plugins(
    ...
    if use_cloudflare_bypass:
        plugins.append(
            EnhancedCloudflarePlugin(
                EnhancedCloudflarePlugin.config(
                    max_wait_time=30,
                    auto_solve=True,
                )
            )
