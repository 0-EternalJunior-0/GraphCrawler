...
class EasyDistributedCrawler:
    ...
    def _setup(self):
        ...
        logger.info("Broker: %s", self.config.broker.url)
        logger.info(
            f"Database: {self.config.database.type}"
            + (f" at {self.config.database.host}" if self.config.database.host else " (local RAM)")
        )
        logger.info("Workers: %s", self.config.workers)
    ...
    def crawl(self, validate: bool = True, skip_worker_check: bool = False) -> Graph:
        ...
        if validate:
            ...
            if not ok:
                ...
                for err in errors:
                    error_msg += f"   {err}\n"
                error_msg += "\nTo fix:\n"
                error_msg += "  1. Start Redis: docker run -d -p 6379:6379 redis:alpine\n"
        ...
        logger.info("Starting distributed crawl for %s URLs", len(self.config.crawl_task.urls))

        # Запускаємо CelerySpider
        graph = self._spider.crawl()

        logger.info("Crawl completed: %s nodes", len(graph.nodes))
        return graph
