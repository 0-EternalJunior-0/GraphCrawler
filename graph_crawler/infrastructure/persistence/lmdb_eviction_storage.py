...
class LMDBEvictionStorage:
    ...
    def save_nodes_sync(self, nodes: List[Any]) -> int:
        ...
        logger.debug("LMDB: Evicted %s nodes to disk", len(nodes))
        return len(nodes)
    ...
    def save_edges_sync(self, edges: List[Any]) -> int:
                ...
                value = pickle.dumps(
                    {
                        ...
                        if not self.lightweight_mode and hasattr(edge, "metadata")
                        else {},
                    }
                )
        ...
        logger.debug("LMDB: Saved %s edges to eviction storage", len(edges))
        return len(edges)
    ...
    def load_node_sync(self, url: str) -> Optional[Dict[str, Any]]:
        ...
        with self.env.begin(db=self.nodes_db) as txn:
            ...
            if value:
                ...
                logger.debug("LMDB: Loaded evicted node: %s", url)
                return data
        return None
    ...
    def load_nodes_batch_sync(self, urls: List[str]) -> Dict[str, Dict[str, Any]]:
        ...
        logger.debug("LMDB: Batch loaded %s evicted nodes", len(result))
        return result
        ...
        if delete_files:
            ...
            try:
                if self.storage_path.exists():
                    ...
                    logger.info("LMDB: Eviction storage deleted: %s", self.storage_path)
            except Exception as e:
                logger.warning("LMDB: Failed to delete eviction storage: %s", e)
