...
class HTMLParser(BaseHTMLParser):
    ...
    _selectolax_available: Optional[bool] = None
    ...
    def _detect_backends(cls):
        ...
        logger.info(" Using HTML parser backend: %s", cls._backend)
    ...
    def _extract_links_from_html(self, html: str, base_url: Optional[str] = None) -> List[str]:
        ...
        if self._native_available:
            try:
                ...
                logger.debug("Native parser failed: %s", e)
        ...
        if self._selectolax_available:
            try:
                ...
                logger.debug("selectolax parser failed: %s", e)
        ...
        for a in tree.find_all("a", href=True):
            href = a.get("href", "")
            if href and isinstance(href, str) and href not in seen and not _is_special_link(href):
                seen.add(href)
                links.append(href)
        return links
