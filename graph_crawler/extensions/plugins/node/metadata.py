...
class MetadataExtractorPlugin(BaseNodePlugin):
    ...
    def execute(self, context: NodePluginContext) -> NodePluginContext:
        ...
        if context.skip_metadata_extraction:
            logger.debug("Skipping metadata extraction for %s", context.url)
            return context

        # Перевірка наявності parser
        if not context.parser:
            logger.warning("No parser for metadata extraction: %s", context.url)
            return context
        ...
        try:
            ...
            if title_elem := parser.find("title"):
                if title_text := title_elem.text():
                    context.set_metadata("title", self._sanitize_text(title_text, MAX_TITLE_LENGTH))
            ...
            if keywords := meta_tags.get("keywords"):
                context.set_metadata("keywords", self._sanitize_text(keywords, MAX_KEYWORDS_LENGTH))

            # H1 - шукаємо в основному контенті, ігноруючи modals/popups/navigation
            h1_text = self._extract_main_h1(parser)
            if h1_text:
                context.set_metadata("h1", self._sanitize_text(h1_text, MAX_H1_LENGTH))
            # Canonical URL
            ...
            logger.debug("Extracted metadata for %s: %s", context.url, list(context.metadata.keys()))

        except Exception as e:
            logger.error("Error extracting metadata for %s: %s", context.url, e)
    ...
    def _extract_main_h1(self, parser: Any) -> Optional[str]:
        ...
        try:
            ...
            logger.debug("Error extracting main H1: %s", e)
    ...
    def _extract_canonical(self, parser: Any, base_url: str) -> Optional[str]:
        ...
        try:
            ...
            logger.debug("Error extracting canonical URL: %s", e)
        return None
    ...
    def _extract_robots_directives(self, meta_tags: Dict[str, str]) -> Dict[str, Any]:
        ...
        if robots_content:
            robots_data["content"] = self._sanitize_text(robots_content, MAX_ROBOTS_CONTENT_LENGTH)
        ...
        if googlebot_content:
            robots_data["googlebot"] = {
                "content": self._sanitize_text(googlebot_content, MAX_ROBOTS_CONTENT_LENGTH),
        ...
        if googlebot_news:
            robots_data["googlebot_news"] = {
                "content": self._sanitize_text(googlebot_news, MAX_ROBOTS_CONTENT_LENGTH),
        ...
        if bingbot_content:
            robots_data["bingbot"] = {
                "content": self._sanitize_text(bingbot_content, MAX_ROBOTS_CONTENT_LENGTH),
    ...
    def _extract_hreflang(self, parser: Any, base_url: str) -> List[Dict[str, str]]:
        ...
        try:
            ...
            for link in alternate_links:
                ...
                if hreflang and href:
                    ...
                    hreflang_list.append(
                        {"hreflang": hreflang.strip().lower(), "href": absolute_href}
                    )
            ...
            logger.debug("Error extracting hreflang: %s", e)
    ...
    def _extract_language(self, parser: Any) -> Optional[str]:
        ...
        try:
            ...
            logger.debug("Error extracting language: %s", e)
        return None
