...
from graph_crawler import AsyncCrawler, async_crawl, crawl
...
class TestFullCrawlWithStorageE2E:
    ...
    def test_crawl_and_save_to_json(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            # Конвертуємо Graph -> GraphDTO перед збереженням
            graph_dto = GraphMapper.to_dto(graph)

            import asyncio
            asyncio.run(storage.save_graph(graph_dto))
            ...
            loaded_graph_dto = asyncio.run(storage.load_graph())

            assert loaded_graph_dto is not None
            assert len(loaded_graph_dto.nodes) == len(graph.nodes)
    ...
    def test_crawl_and_save_to_sqlite(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            assert loaded_graph_dto is not None
            assert len(loaded_graph_dto.nodes) > 0
