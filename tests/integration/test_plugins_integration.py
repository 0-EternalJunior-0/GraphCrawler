"""Integration tests для CustomPlugins системи.

Тести перевіряють роботу CustomPlugins з реальним краулінгом.
"""
...
from graph_crawler import async_crawl, crawl
from graph_crawler.extensions.plugins.node import BaseNodePlugin, NodePluginContext, NodePluginType

@pytest.mark.integration
class TestPluginsBasicIntegration:
    """Базові integration тести для CustomPlugins."""
    ...
    def test_multiple_plugins_work_together(self):
        """Декілька CustomPlugins працюють разом."""
        events_plugin1 = []
        events_plugin2 = []
        ...
        # Обидва CustomPlugins отримали events
        assert len(events_plugin1) >= 1
        assert len(events_plugin2) >= 1

        # Обидва CustomPlugins додали metadata
        root = list(graph.nodes.values())[0]
...
class TestPluginsAsyncIntegration:
    """Async integration тести для CustomPlugins."""
...
class TestPluginsWithRealCrawling:
    """Тести CustomPlugins з реальним краулінгом."""
    ...
    def test_plugin_tracks_all_discovered_urls(self):
        ...
        # Plugin може викликатися для кожного знайденого URL (включаючи дублікати з різних сторінок)
        # Перевіряємо що унікальні URL в discovered >= кількості nodes в графі
        unique_discovered_urls = set(item["url"] for item in discovered)
        assert len(unique_discovered_urls) >= len(graph.nodes)
    ...
    def test_plugin_can_filter_urls(self):
        ...
        [n for n in graph.nodes.values() if n.user_data.get("blocked")]
...
class TestPluginsCustomBehavior:
    """Тести custom поведінки CustomPlugins."""
...
class TestPluginsErrorHandling:
    """Тести обробки помилок в CustomPlugins."""

    def test_plugin_error_does_not_crash_crawler(self):
        """Помилка в plugin не ламає package_crawler."""
