...
class TestSpiderWithHTTPDriver:
    ...
    async def test_spider_crawls_with_http_driver(self):
        """Spider краулить з HTTP driver і повертає GraphDTO."""
        config = CrawlerConfig(
            url="https://books.toscrape.com/",
            max_depth=0,
            max_pages=1
        ...
        # Spider.crawl() повертає GraphDTO
        assert isinstance(graph, GraphDTO)
        # GraphDTO.nodes - це список NodeDTO
        scanned_nodes = [n for n in graph.nodes if n.scanned]
        assert len(scanned_nodes) == 1

        root = [n for n in graph.nodes if n.scanned][0]
        assert root.scanned is True
        # URL нормалізується без trailing slash
        assert "books.toscrape.com" in root.url
...
class TestSpiderDriverFeatures:
    ...
    async def test_spider_extracts_links_correctly(self):
        ...
        # graph.nodes може бути list (DTO) або dict (Entity)
        node_ids = set()
...
class TestSpiderConcurrency:
    ...
    async def test_spider_handles_concurrent_same_site(self):
        ...
        spiders = [
            ...
            for config, driver, storage in zip(configs, drivers, storages, strict=False)
        ]
