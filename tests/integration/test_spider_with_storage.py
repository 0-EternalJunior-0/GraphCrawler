...
NOTE: Spider.crawl() повертає GraphDTO (не Domain Graph) - це архітектурне рішення.
Публічний API використовує DTO для безпечного обміну даними.
"""
...
from graph_crawler.shared.dto.graph_dto import GraphDTO
from graph_crawler.domain.value_objects.configs import CrawlerConfig
from graph_crawler.infrastructure.persistence import JSONStorage, MemoryStorage, SQLiteStorage
...
class TestSpiderWithMemoryStorage:
    ...
    async def test_spider_with_memory_storage_basic(self):
        """Spider працює з memory storage і повертає GraphDTO."""
        ...
        # Spider.crawl() повертає GraphDTO (архітектурне рішення)
        assert isinstance(graph_dto, GraphDTO)
        # Перевіряємо що scanned тільки 1 node (root)
        scanned_nodes = [n for n in graph_dto.nodes if n.scanned]
...
class TestSpiderWithJSONStorage:
    ...
    async def test_spider_with_json_storage_basic(self, tmp_path):
        ...
        # GraphDTO.nodes - це список NodeDTO
        scanned_nodes = [n for n in graph.nodes if n.scanned]
        assert len(scanned_nodes) == 1
    ...
    async def test_spider_loads_from_json_storage(self, tmp_path):
        ...
        # Порівнюємо URLs (GraphDTO.nodes - це список)
        original_urls = {n.url for n in graph.nodes}
        loaded_urls = {n.url for n in loaded_graph.nodes}
...
class TestSpiderStorageComparison:
    ...
    async def test_storage_types_produce_same_graph(self, tmp_path):
        ...
        urls_mem = {n.url for n in loaded_mem.nodes.values()}
        urls_json = {n.url for n in loaded_json.nodes.values()}
        urls_sqlite = {n.url for n in loaded_sqlite.nodes.values()}
...
class TestSpiderStorageEdgeCases:
    ...
    async def test_storage_handles_empty_graph(self, tmp_path):
        ...
        # Graph вже імпортований на рівні модуля
