...
class TestGraphURLNormalization:
    """Тести для нормалізації URL (browser-like behavior)."""

    def test_normalize_url_without_trailing_slash(self):
        """https://example.com залишається без trailing slash."""
        graph = Graph()
        node = Node(url="https://example.com", depth=0)
        graph.add_node(node)

        # Пошук без trailing slash
        result = graph.get_node_by_url("https://example.com")
        assert result is not None
        assert result.url == "https://example.com"

    def test_normalize_url_with_trailing_slash(self):
        """https://example.com/ нормалізується до https://example.com."""
        graph = Graph()
        node = Node(url="https://example.com/", depth=0)
        graph.add_node(node)

        # Пошук обома варіантами
    ...
    def test_normalize_mixed_slashes(self):
        """https://example.com////////////////\\\\///// нормалізується."""
        graph = Graph()
        weird_url = "https://example.com////////////////\\\\/////"
        node = Node(url=weird_url, depth=0)
        graph.add_node(node)

        # Всі варіації повинні знаходити ту саму ноду
        assert graph.get_node_by_url(weird_url) is not None
        assert graph.get_node_by_url("https://example.com") is not None
        assert graph.get_node_by_url("https://example.com/") is not None

    def test_normalize_path_with_trailing_slash(self):
        """https://example.com/page/ нормалізується до https://example.com/page."""
        graph = Graph()
        node = Node(url="https://example.com/page/", depth=0)
        graph.add_node(node)

        assert graph.get_node_by_url("https://example.com/page") is not None
        assert graph.get_node_by_url("https://example.com/page/") is not None

    def test_normalize_path_multiple_slashes(self):
        """https://example.com/path////////////////\\\\///// нормалізується."""
        graph = Graph()
        weird_url = "https://example.com/path////////////////\\\\/////"
        node = Node(url=weird_url, depth=0)
        graph.add_node(node)

        assert graph.get_node_by_url("https://example.com/path") is not None
        assert graph.get_node_by_url(weird_url) is not None

    def test_normalize_uppercase_domain(self):
        """HTTPS://EXAMPLE.COM нормалізується до https://example.com."""
        graph = Graph()
        node = Node(url="https://EXAMPLE.COM", depth=0)
        graph.add_node(node)
    ...
    def test_contains_with_normalization(self):
        """in operator працює з нормалізацією URL."""
        graph = Graph()
        node = Node(url="https://example.com/", depth=0)
        graph.add_node(node)
    ...
    def test_getitem_with_normalization(self):
        """graph[url] працює з нормалізацією URL."""
        graph = Graph()
        node = Node(url="https://example.com/page/", depth=0)
        graph.add_node(node)

        assert graph["https://example.com/page"] is node
        assert graph["https://example.com/page/"] is node

    def test_duplicate_urls_normalize_to_same(self):
        """Дублікати з різними формами URL нормалізуються до одного."""
        graph = Graph()
        node1 = Node(url="https://example.com", depth=0)
        node2 = Node(url="https://example.com/", depth=1)
        node3 = Node(url="https://EXAMPLE.COM", depth=2)
        ...
        assert len(graph) == 1
        # Перша нода залишається
        assert graph.get_node_by_url("https://example.com").depth == 0

    def test_is_url_known_with_normalization(self):
        """is_url_known() працює з нормалізацією URL."""
        graph = Graph()
        node = Node(url="https://example.com/page/", depth=0)
        graph.add_node(node)

        assert graph.is_url_known("https://example.com/page") is True
        assert graph.is_url_known("https://example.com/page/") is True
        assert graph.is_url_known("https://example.com/other") is False

    def test_get_url_status_with_normalization(self):
        """get_url_status() працює з нормалізацією URL."""
        graph = Graph()
        node = Node(url="https://example.com/", depth=0)
        graph.add_node(node)

        known, found_node = graph.get_url_status("https://example.com")
