...
class TestGraphSpiderCrawl:
    ...
    async def test_crawl_single_page(self):
        ...
        # Mock _coordinator напряму (внутрішній атрибут)
        if hasattr(spider, '_coordinator') and spider._coordinator is not None:
            with patch.object(spider._coordinator, 'coordinate', new_callable=AsyncMock) as mock_coord:
                mock_coord.return_value = spider.graph
    ...
    async def test_crawl_emits_start_event(self):
        ...
        if hasattr(spider, '_coordinator') and spider._coordinator is not None:
            ...
            # Створюємо і публікуємо подію вручну для тесту використовуючи factory метод
            from graph_crawler.domain.events import CrawlerEvent
            event = CrawlerEvent.create(event_type=EventType.CRAWL_STARTED, data={})
...
class TestGraphSpiderErrorHandling:
    ...
    async def test_handles_driver_error(self):
        ...
        # Патчимо внутрішній атрибут _coordinator (coordinator є property)
        mock_coordinator = AsyncMock()
        mock_coordinator.coordinate = AsyncMock(return_value=spider.graph)
        spider._coordinator = mock_coordinator
