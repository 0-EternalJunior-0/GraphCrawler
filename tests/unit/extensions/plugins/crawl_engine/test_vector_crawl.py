...
NOTE: Деякі тести потребують ML залежностей (sentence-transformers, scikit-learn).
Встановіть їх через: pip install sentence-transformers scikit-learn
"""

import pytest

# Перевірка наявності ML залежностей
try:
...
class TestVectorCrawlEnginePluginBatch:
    ...
    def test_batch_priorities(self, plugin):
        ...
        for _url, priority in result.items():
            assert plugin.min_priority <= priority <= plugin.max_priority
