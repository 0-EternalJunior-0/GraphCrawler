...
class TestPostScanHooks:
    ...
    async def test_single_hook_modifies_links(self):
        ...
        # Mock scanner - повертає tuple (links, fetch_response)
        scanner = AsyncMock()
...
class TestIntegration:
    ...
    async def test_all_mechanisms_together(self):
        ...
        # Hook модифікує ноду в графі, тому перевіряємо через граф
        source_in_graph = result_graph.get_node_by_url("https://example.com")
        assert source_in_graph.user_data.get('explicit_scan_decisions') is not None
